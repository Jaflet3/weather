...
API_KEY = "your_weatherapi_key"  # Replace with your key
BASE_URL = "http://api.weatherapi.com/v1"

st.title("🌦️ Weather Info App")

city = st.text_input("City", "New York")
date_input = st.text_input("Date (YYYY-MM-DD)", datetime.today().strftime("%Y-%m-%d"))
...
if st.button("Get Weather"):
    try:
        ...
        if user_date.date() < now.date():
            endpoint = "history.json"
        elif user_date.date() == now.date():
        ...
        if endpoint == "current.json":
            ...
            st.write(f"Condition: {current.get('condition', {}).get('text', '-')}")
            st.write(f"Temperature: {current.get('temp_c')} °C")

        elif endpoint == "history.json":
            hist = data["forecast"]["forecastday"][0]["day"]
            st.write(f"Avg Temp: {hist['avgtemp_c']} °C")
            st.write(f"Condition: {hist['condition']['text']}")

        elif endpoint == "forecast.json":
            fore = data["forecast"]["forecastday"][0]["day"]
            st.write(f"Max Temp: {fore['maxtemp_c']} °C")
            st.write(f"Condition: {fore['condition']['text']}")
